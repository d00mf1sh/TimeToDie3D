	...
	public class plyRPG_UI : MonoBehaviour
	{
		...
		public void UpdateActionBar()
		{
			...
			for (int slotIdx = 0; slotIdx < 10; slotIdx++)
			{
				...
				if (Player.Instance.actionSlotCount > slotIdx)
				{
					...
					UnityEngine.UI.Image img = buttonTr.GetChild(0).GetComponent<UnityEngine.UI.Image>();
					...
					if (slotType == ActionSlots.SlotType.Skill)
					{
						...
						if (sk != null)
						{
							img.sprite = sk.def.sprites[0]; // expect image to be in first slot of definition image property
							img.gameObject.SetActive(img.sprite != null); // hide image if null
						}
					}
					...
					else if (slotType == ActionSlots.SlotType.Item)
					{
						...
						if (it != null)
						{
							img.sprite = it.def.sprites[0]; // expect image to be in first slot of definition image property
							img.gameObject.SetActive(img.sprite != null); // hide image if null
						}
					}

					else if (slotType == ActionSlots.SlotType.None)
					{	// Make sure the slot is cleared
						img.sprite = null;
		...
		public void RefreshBag()
		{
			...
			for (int i = 0; i < container.childCount; i++)
			{
				...
				UnityEngine.UI.Image img = slotObj.GetChild(0).GetComponent<UnityEngine.UI.Image>();	// i know the 1st child is the image
				...
				else
				{
					...
					img.sprite = iib.item.def.sprites[0];
		...
		private void UpdateEquipSlots()
		{
			...
			for (int i = 0; i < slots.items.Count; i++)
			{
				...
				// update the image
				UnityEngine.UI.Image img = buttonTr.GetChild(0).GetComponent<UnityEngine.UI.Image>();
				img.sprite = slots.items[i].item.def.sprites[0];
				img.gameObject.SetActive(true);
			}
		}
		...
		public void UpdateSkillsPanel()
		{
			...
			for (int i = 0; i < Player.Instance.actor.knownSkills.Count; i++)
			{
				...
				{
					go.transform.GetChild(0).GetComponent<UnityEngine.UI.Image>().sprite = Player.Instance.actor.knownSkills[i].def.sprites[0];
				}
